...
- [2.1 Partition Leadership, Replication, HA](#21-partition-leadership-replication-ha)
  - [Single-leader model](#single-leader-model)
  - [Write durability knobs](#write-durability-knobs)
- [2.2 Storage Internals: Segments, Indexes, Retention](#22-storage-internals-segments-indexes-retention)
  - [Segmented logs](#segmented-logs)
  - [Retention policies](#retention-policies)
- [2.3 Log Cleanup: Delete vs Compaction](#23-log-cleanup-delete-vs-compaction)
  - [Choosing policies](#choosing-policies)
  - [Example: topic configs](#example-topic-configs)
- [2.4 Metadata Control Plane: ZooKeeper vs KRaft](#24-metadata-control-plane-zookeeper-vs-kraft)
  - [Evolution](#evolution)
  - [Why it matters](#why-it-matters)
- [2.5 Rebalancing and Coordination](#25-rebalancing-and-coordination)
  - [Consumer group rebalancing](#consumer-group-rebalancing)
  - [Enabling cooperative rebalancing](#enabling-cooperative-rebalancing)
- [2.6 Designing for Reliability](#26-designing-for-reliability)
  - [Replication and ISR](#replication-and-isr)
  - [Rack awareness](#rack-awareness)
- [2.7 Performance Implications](#27-performance-implications)
  - [IO patterns and batching](#io-patterns-and-batching)
  - [Producer and consumer tuning](#producer-and-consumer-tuning)
- [2.8 Practical Checklist](#28-practical-checklist)
  - [Operations](#operations)
  - [Data layout](#data-layout)
- [2.9 Handy Admin and Spring Snippets](#29-handy-admin-and-spring-snippets)
  - [CLI topic creation](#cli-topic-creation)
  - [Spring Topic beans](#spring-topic-beans)
  - [Consumer cooperative assignor (Spring Boot)](#consumer-cooperative-assignor-spring-boot)
...
Each partition in Kafka has one leader and several follower replicas. The leader handles all reads and writes, while the followers replicate the data from the leader. The leader's role is critical for maintaining the consistency of the partition.

### Replication Factor:

This is a configuration setting that determines how many copies of a partition exist across different brokers. For example, a replication factor of 3 means that there will be three copies of each partition.

### ISR List:

The ISR list is a dynamic list of replicas that are in sync with the leader. This list is crucial for determining which replicas are eligible to handle failover scenarios.
